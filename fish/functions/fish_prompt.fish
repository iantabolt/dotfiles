...
function __foursquare_icon
  set -g foursquare_colors f94877 ffffff
  __bobthefish_start_segment $foursquare_colors 
  echo -ns \uf180 ' '
  set_color normal
end
...
function __bobthefish_prompt_hg -S -a current_dir -d 'Display the actual hg state'
  # OOPS BROKE THIS set -l dirty (command ???)
...
function __bobthefish_prompt_git -S -a current_dir -d 'Display the actual git state'
  set -l dirty   (command git diff --no-ext-diff --quiet --exit-code --shortstat; or echo -ns \uf040 ' ')
  set -l staged  (command git diff --cached --no-ext-diff --quiet --exit-code --shortstat; or echo -ns \uf490 ' ')
  set -l stashed (command git rev-parse --verify --quiet refs/stash >/dev/null; and echo -ns \uf019 ' ')
